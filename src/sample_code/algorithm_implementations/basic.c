...
// 2. KORAK: Sekvenčni izračun kumulativne energije (od spodaj navzgor)
void calculate_cumulative_energy(const float *energy, float *cumulative, int w, int h, int orig_w) {
    ...
    // Izračun vrstico po vrstico od spodaj navzgor
    for (int r = h - 2; r >= 0; r--) {
        // Odstranjen #pragma omp parallel for
        for (int c = 0; c < w; c++) {
...
// 3. KORAK: Sekvenčno iskanje poti šiva od zgoraj navzdol (To je bilo sekvenčno že prej)
...
int main(int argc, char *argv[]) {
    ...
    // Ker smo odstranili OpenMP paralelizacijo iz te datoteke, se bo 
    // to izvedlo na eni niti (ali na toliko, kolikor jih nastavi bash okolje).
    print_omp_info();
