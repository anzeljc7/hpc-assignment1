...
void find_multiple_seams(float *cumulative, int *seams, int curr_width, int curr_height, int orig_width, int k)
{
    for (int i = 0; i < k; i++)
    {
        ...
        for (int r = 0; r < curr_height - 1; r++)
        {
            ...
            if (next_c > 0)
                cumulative[(r + 1) * orig_width + next_c - 1] = 1e9f;
            if (next_c < curr_width - 1)
                cumulative[(r + 1) * orig_width + next_c + 1] = 1e9f;
        }
    }
}
